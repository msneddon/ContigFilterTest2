...
from SneddonContigFilter2.SneddonContigFilter2Impl import SneddonContigFilter2


class SneddonContigFilter2Test(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'SneddonContigFilter2',
        ...
        for nameval in config.items('SneddonContigFilter2'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = SneddonContigFilter2(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_SneddonContigFilter2_" + str(suffix)
